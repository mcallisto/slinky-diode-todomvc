...
object AppCircuit extends Circuit[AppModel] {
  ...
  def initialModel: AppModel = AppModel(Todos(Seq(), None, TodoFilter.All))
...
class EditTodoHandler[M](modelRW: ModelRW[M, Option[Todo.Id]]) extends ActionHandler(modelRW) {
  override protected def handle = {
    ...
    case EditingDone(_)      => updated(None)
  }
}
...
class TodoHandler[M](modelRW: ModelRW[M, Seq[Todo]]) extends ActionHandler(modelRW) {
  ...
  override def handle = {
    case InitTodos =>
      updated(List(Todo(Todo.randomId, "Test your code!", isCompleted = false)))
    case AddTodo(title) =>
      updated(value :+ Todo(Todo.randomId, title, isCompleted = false))
